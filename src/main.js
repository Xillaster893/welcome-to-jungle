...
const BITE_SOUND_PATH = "../assets/bite-crunch.wav";
const GAME_OVER_SOUND_PATH = "../assets/gameover.wav";
const BITE_SOUND_POOL_SIZE = 4;
...
function playEatSound() {
  if (isMuted || biteSoundPool.length === 0) {
    return;
  }

  const sound = biteSoundPool[nextBiteSoundIndex];
  nextBiteSoundIndex = (nextBiteSoundIndex + 1) % biteSoundPool.length;
...
function playGameOverSound() {
  ...
  gameOverSound.volume = 0.28;
  gameOverSound.play().catch(() => {});
}

function playTone({ frequency, peakFrequency, duration, gain, type }) {
  if (isMuted) {
    return;
  }
  ...
  oscillator.frequency.exponentialRampToValueAtTime(Math.max(peakFrequency, 1), endTime);

  gainNode.gain.setValueAtTime(0.0001, startTime);
  gainNode.gain.exponentialRampToValueAtTime(gain, startTime + duration * 0.18);
...
function createBiteSoundPool() {
  biteSoundPool = Array.from({ length: BITE_SOUND_POOL_SIZE }, () => {
    const audio = new Audio(BITE_SOUND_PATH);
    audio.preload = "auto";
    audio.volume = 0.18;
    return audio;
  });
...
function createGameOverSound() {
  gameOverSound = new Audio(GAME_OVER_SOUND_PATH);
  gameOverSound.preload = "auto";
  gameOverSound.volume = 0.28;
}

function primeBiteSound() {
  if (biteSoundPool.length === 0) {
    return;
  }

  biteSoundPool[0].load();
}
